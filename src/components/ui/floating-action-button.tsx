  ...
  const actions: SpeedDialAction[] = [
    {
      ...
      onClick: () => {
        ...
        // Later, this could navigate or open a modal:
    ...
    },
    {
      ...
      onClick: () => {
        ...
        // Open payment modal or navigate
      },
    },
  ];

  if (!isMounted) {
    return null; // Avoid rendering on the server to prevent hydration issues with fixed positioning
  }
            ...
            <div key={action.id} className="group relative flex items-center">
              <span
                className={cn(
                  ...
                  !isOpen && 'hidden' // Hide tooltips when FAB is closed to prevent overlap issues
                )}
              >
                {action.label}
              </span>
              <Button
                ...
                onClick={() => {
                  action.onClick?.();
                  setIsOpen(false); // Close speed dial after action
                }}
